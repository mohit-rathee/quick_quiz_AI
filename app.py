...
def start_quiz():
    ...
    with get_db() as db:
        cur = db.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute(
            "INSERT INTO quiz_sessions (user_id, topic, total) VALUES (%s,%s,%s) RETURNING id",
...
def submit_answer():
    ...
    with get_db() as db:
        cur = db.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute(
            "INSERT INTO answers (session_id, question, selected_text, correct_text, is_correct) VALUES (%s,%s,%s,%s,%s)",
            (session["quiz_session_id"], mcq["question"], selected["text"], correct["text"], int(is_correct))
        )
        if is_correct:
            cur.execute("UPDATE quiz_sessions SET score = score + 1 WHERE id=%s", (session["quiz_session_id"],))
        db.commit()
        cur.execute("SELECT score FROM quiz_sessions WHERE id=%s", (session["quiz_session_id"],))
...
def finish_quiz():
    ...
    with get_db() as db:
        cur = db.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute(
            "UPDATE quiz_sessions SET completed=1, finished_at=%s WHERE id=%s",
            (datetime.now(), sid)
        )
        db.commit()
        cur.execute("SELECT score, total, topic FROM quiz_sessions WHERE id=%s", (sid,))
        row = cur.fetchone()
    ...
    with get_db() as db:
        ...
        if topic_filter:
            cur.execute("""
                ...
                WHERE qs.completed = 1 AND LOWER(qs.topic) LIKE %s
                ORDER BY pct DESC, qs.score DESC
                LIMIT 50
            """, (f"%{topic_filter.lower()}%",))
        else:
            cur.execute("""
                SELECT u.username, qs.topic, qs.score, qs.total,
                       ROUND(qs.score * 100.0 / qs.total) AS pct, qs.finished_at
...
def stats():
    with get_db() as db:
        cur = db.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute("SELECT COUNT(*) AS c FROM quiz_sessions WHERE completed=1")
